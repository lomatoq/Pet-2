...
fn den_orb_rest_position(anchor: Vec2, scale: f32, viewport: [u32; 2], radius: f32) -> Vec2 {
    ...
    // One fixed physical/render radius across storage, pickup and release.
    let visible_radius = radius / REFERENCE_DESKTOP_HEIGHT_PX;
    (anchor + Vec2::Y * (seat - visible_radius)).clamp(Vec2::ZERO, Vec2::ONE)
}

fn settle_stored_orb(object: &mut WorldObject, target: Vec2, aspect: f32, dt: f32) {
    let dt = if dt.is_finite() {
        dt.clamp(0.0, 1.0 / 30.0)
    } else {
        0.0
    };
    let metric = Vec2::new(aspect, 1.0);
    let error = (object.position - target) * metric;
    let spring = object.velocity + error * 7.0;
    let decay = (-7.0 * dt).exp();
    object.position = target + (error + spring * dt) * decay / metric;
    object.velocity = (object.velocity - spring * (7.0 * dt)) * decay;
    if (object.position - target).length() < 0.00001 && object.velocity.length() < 0.0001 {
...
impl EcologyRuntime {
    ...
    pub(crate) fn orb_physical_frame(
        ...
        // Once attached, the socket belongs to the body. Following the orb's
        // current normal every decision tick makes gravity move the socket too,
        ...
        let surface_px = contact.filter(|_| grip.is_none()).map_or_else(
        ...
        let socket_px = grip.map_or(surface_px + normal_px * radius_px * 0.65, |grip| {
            let offset = grip.target - grip.body_origin;
            Vec2::new(offset.x * aspect, offset.y) * viewport_height
        });
    ...
    pub fn settle_food_body(&mut self, body: &mut ProceduralBody, height: f32, dt: f32) -> bool {
        ...
        if !self.food_grounded && gap.abs() <= 2.0 {
            self.food_grounded = true;
            self.food_support_root_y = Some(center.y + gap / height);
        }
        // A landing establishes one support frame. Chasing the changing render
        // skirt every tick moves the physical plane into the particles again,
        // ratcheting the root below the floor and feeding energy into the gel.
        let step = if self.food_grounded {
            (self.food_support_root_y.unwrap_or(center.y) - center.y) * height
        } else {
    ...
    pub fn update_food_contact(&mut self, body: &ProceduralBody, height: f32) -> Option<Vec2> {
        ...
        let Some(food) = food else {
            if let Some((_, rest)) = self.food_rest {
                let center = body.simulation.feedback.world_position;
        ...
        if food.lifecycle == ObjectLifecycle::Sleeping {
            self.food_rest = Some((food.id, Vec2::new(food.position.x, self.food_floor(food))));
        }
        let settled = self.food_rest.is_some_and(|(id, _)| id == food.id);
    ...
    pub fn fixed_update(
        ...
        let den_scale = self.state.den.size_scale;
        let den_viewport = self.den_viewport;
        ...
        for object in &mut self.state.objects {
            let before_motion = object.position;
            let resting_position = den_orb_rest_position(
                den_anchor,
                den_scale,
                den_viewport,
                object.radius_px_at_reference,
            );
            if object.lifecycle == ObjectLifecycle::StoredInDen {
                let previous = object.position;
                settle_stored_orb(object, resting_position, aspect, dt);
                crate::orb_cradle::constrain(object, previous, den_anchor, den_scale, den_viewport);
                if object.kind == ObjectKind::Orb {
                    self.environment.orb_position = Some(object.position);
                    self.environment.orb_grounded = true;
                }
                continue;
            }
                ...
                .find(|grip| grip.object_id == object.id)
            {
                ...
                // Damping is relative to the carrier, otherwise a moving pet
                // continuously drags against its own grip's world-space brake.
                let carrier_velocity = Vec2::new(body.velocity.x * aspect, body.velocity.y);
                step_compliant_grip(object, target, carrier_velocity, grip.strength, config, dt);
                if dt.is_finite()
                    && dt > 0.0
            ...
            let before_attraction = object.position;
            ...
            let latch_requested = !self.release_grace.iter().any(|(id, _)| *id == object.id)
                && step_den_attraction(object, resting_position, config, dt);
            crate::orb_cradle::constrain(
                object,
                before_attraction,
                den_anchor,
            ...
            if latch_requested
                && desktop_distance(object.position, resting_position, aspect)
                    * REFERENCE_DESKTOP_HEIGHT_PX
                    < 0.05
        ...
        if let Some((position, speed, lifecycle, radius)) = current_orb {
            let field_center = den_orb_rest_position(den_anchor, den_scale, den_viewport, radius);
            let field_now = desktop_distance(position, field_center, self.desktop_aspect)
                * reference_height
    ...
    pub fn observe_pointer(
                ...
                .find(|object| object.id == object_id)
        {
            ...
            self.drag_velocity = object.velocity;
    ...
    pub fn set_den_anchor(&mut self, anchor: Vec2) {
        let delta = anchor - self.state.den.anchor;
        for object in &mut self.state.objects {
            if object.lifecycle == ObjectLifecycle::StoredInDen {
                object.position = (object.position + delta).clamp(Vec2::ZERO, Vec2::ONE);
            }
        }
...
mod tests {
    ...
    fn exercise_floor_food_landing(height: f32, gap: f32, crumbs: usize, seed: u64) {
        ...
        let mut previous_position = body.simulation.feedback.world_position;
        let mut maximum_face_step = 0.0_f32;
        let mut previous_mouth = body.feeding_mouth_rest_pixels(height);
        ...
        for tick in 0..3600 {
            ...
            let target = runtime.update_food_contact(&body, height);
            body.set_feeding_mouth(target, height, 1.0 / 120.0);
            ...
            maximum_face_step = maximum_face_step.max(mouth.distance(previous_mouth));
            previous_mouth = mouth;
            if runtime.food_grounded && tick % 30 == 0 {
                let hull = body.main_liquid_contact_bounds_pixels(height);
                let floor_y = (0.96 - body.simulation.feedback.world_position.y) * height;
        ...
        assert!(!runtime.settle_food_body(&mut body, height, 1.0 / 120.0));
        assert_eq!(body.simulation.feedback.world_position, lifted);
        assert!(runtime.feeding_support().is_none());
    }
    ...
    fn gentle_tap_leaves_den_before_capture_is_reenabled() {
        ...
        runtime.state.objects[0].position = den_orb_rest_position(
            start,
            runtime.state.den.size_scale,
            runtime.den_viewport,
            runtime.state.objects[0].radius_px_at_reference,
        );
        runtime.state.objects[0].velocity = Vec2::ZERO;
        runtime.fixed_update(16.0 / 9.0, &windows, &body, 1.0 / 120.0);
        assert_eq!(
    ...
    #[test]
    fn familiar_orb_is_carried_during_ordinary_low_drive_self_play() {
        let directory = tempfile::tempdir().unwrap();
        let store = StateStore::at(directory.path());
        let seed = 5784121873664838231;
        let mut runtime = EcologyRuntime::load_or_create(&store, seed, true).unwrap();
    ...
    #[test]
    fn repeated_grab_commands_carry_orb_without_socket_creep() {
        for aspect in [1.0, 2.4] {
            ...
            runtime.desktop_aspect = aspect;
            runtime.state.den.anchor = Vec2::splat(0.95);
            let genome = lifecore::Genome::from_seed(74);
            let mut body = ProceduralBody::generate(&genome).unwrap();
            ...
            let mut feedback = BodyFeedback {
                world_position: Vec2::new(0.3, 0.4),
                velocity: Vec2::new(0.065 / aspect, 0.0),
                ..Default::default()
            };
            let support = body.liquid_physical_support_pixels(Vec2::X, 1080.0);
            let orb = &mut runtime.state.objects[0];
            ...
            let start = orb.position;
            let id = orb.id;
            runtime.last_body_position = feedback.world_position;
            let initial = runtime.orb_physical_frame(&body, &feedback, 1080.0, 1.0 / 120.0);
            assert!(initial.contact);
    ...
    fn remote_orb_move_cannot_start_a_grip() {
        ...
        assert!(runtime.pet_grips.is_empty());
        assert_eq!(runtime.state.den.slots[0], Some(id));
        assert_eq!(runtime.state.objects[0].home_slot, Some(0));
    }
    ...
    fn compliant_pickup_is_continuous_and_fast_release_escapes_den() {
        ...
        runtime.last_orb_physical = PhysicalGrabFrame {
            contact: true,
            socket_position: start - Vec2::new(0.0, 0.10),
            ..PhysicalGrabFrame::default()
        };
        output.object_commands[0] = ObjectCommand::MoveToward {
            object_id: id,
            target: start - Vec2::new(0.0, 0.10),
            speed: 5.0,
        };
        runtime.apply_object_commands(&output, 0.05);
        assert_eq!(
        ...
        // A compliant grip carries weight: vertical equilibrium includes spring sag.
        let sag = pet_ecology::ORB_SCREEN_GRAVITY * 0.4 * runtime.state.objects[0].mass
            / (8.5_f32.powi(2) * 0.72);
        assert!(
            (runtime.state.objects[0].position.y - (start.y - 0.10 + sag)).abs() < 0.003,
            "pickup position {:?}, target {:?}",
    ...
    #[test]
    fn released_orb_inside_den_is_pulled_smoothly_to_cushion_then_latched() {
        let directory = tempfile::tempdir().unwrap();
        let store = StateStore::at(directory.path());
        ...
        assert_eq!(
            runtime.state.objects[0].position,
            den_orb_rest_position(
                runtime.state.den.anchor,
                runtime.state.den.size_scale,
                runtime.den_viewport,
                runtime.state.objects[0].radius_px_at_reference,
            )
    ...
    #[test]
    fn stored_orb_settles_on_visible_cushion_without_a_grab_coordinate_jump() {
        for viewport in [[1920, 1080], [3440, 1440], [800, 1280]] {
            let directory = tempfile::tempdir().unwrap();
            let store = StateStore::at(directory.path());
            let mut runtime = EcologyRuntime::load_or_create(&store, 812, true).unwrap();
            runtime.set_den_viewport(viewport);
            runtime.state.den.anchor = Vec2::new(0.7, 0.8);
            let anchor = runtime.state.den.anchor;
            let orb = &mut runtime.state.objects[0];
            orb.position = anchor;
            orb.velocity = Vec2::ZERO;
            orb.lifecycle = ObjectLifecycle::StoredInDen;
            orb.home_slot = Some(0);
            runtime.state.den.slots[0] = Some(orb.id);
            let aspect = viewport[0] as f32 / viewport[1] as f32;
            let body = BodyFeedback {
                world_position: Vec2::splat(0.2),
                ..Default::default()
            };
            let windows = WindowAffordanceFrame::default();
            ...
            for _ in 0..480 {
                runtime.fixed_update(aspect, &windows, &body, 1.0 / 120.0);
                let current = runtime.state.objects[0].position;
                assert!((current - previous).length() * (viewport[1] as f32) < 1.0);
                previous = current;
            }
            let orb = &runtime.state.objects[0];
            let height = viewport[1] as f32;
            let visible_bottom = (orb.position.y - anchor.y) * height
                + orb.radius_px_at_reference / REFERENCE_DESKTOP_HEIGHT_PX * height;
            let seat = pet_body::birth_scene::den_seat_depth_pixels(
                viewport,
                runtime.state.den.size_scale,
            );
            assert!((visible_bottom - seat).abs() < 0.05);
            let physical_center = orb.position;
            assert!(runtime.hit_test(physical_center, aspect, height, 0.0));
            assert!(runtime.observe_pointer(
                Some(physical_center),
                true,
                true,
                aspect,
                height,
            ...
            assert_eq!(runtime.state.objects[0].position, physical_center);
        }
    }
    ...
    fn object_commands_cannot_leave_stale_or_duplicate_den_slots() {
        ...
        let restore_stored = |runtime: &mut EcologyRuntime| {
            runtime.last_orb_physical = PhysicalGrabFrame {
                contact: true,
                socket_position: runtime.state.den.anchor,
                ..PhysicalGrabFrame::default()
            };
