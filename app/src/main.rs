...
use pet_motor::{
    ...
    MotorWorldGoal, SomaticActuationBus, SomaticActuationPacket, SurfaceCandidate,
    VoiceSemanticIntent,
};
use pet_perception::{
        ...
        *morph_command_counts
        ...
        let ecology_output = ecology.resolve_intent(
            ...
            EcologyResolveFrame {
                ...
                dt,
                orb_physical: pet_ecology::PhysicalGrabFrame {
                    socket_position: feedback.world_position,
                    ..pet_ecology::PhysicalGrabFrame::default()
                },
            },
        );
...
impl PetApplication {
    ...
    fn update_runtime(&mut self, event_loop: &ActiveEventLoop) {
        ...
        while runtime.life_accumulator >= LIFE_DT {
            ...
            // Phase timing and the bout's locked target own the final semantic
            // body performance. This remains downstream of goal selection and
            // never feeds a second action choice back into LifeCore.
...
fn build_motor_context(runtime: &mut PetRuntime) -> BehaviorContextFrame {
    let body = runtime.nervous_system.body_feedback();
    let somatic = runtime.body.somatic_feedback();
    let overlay_height = runtime.window.inner_size().height.max(1) as f32;
    let desktop_height = runtime.topology.virtual_physical_bounds.height().max(1) as f32;
    let liquid_bounds = runtime.body.liquid_visual_bounds_pixels(overlay_height);
    let body_bottom_extent =
    ...
    let world_event = runtime
        .ecology
        .take_den_event()
        .unwrap_or(MotorWorldEvent::None);
    let state = runtime.ecology.state();
    let orb = state
        .objects
        .iter()
        .find(|object| object.kind == ObjectKind::Orb);
    let mut surfaces: Vec<_> = runtime
        .sensors
        .visible_surfaces
        .iter()
        .map(|surface| SurfaceCandidate {
            surface_id: surface.id.clone(),
            minimum: surface.rect.minimum,
            maximum: surface.rect.maximum,
            velocity: Vec2::ZERO,
            familiarity: if runtime.intent.target_surface.as_ref() == Some(&surface.id) {
                0.55
            } else {
                0.20
            },
        ...
        .collect();
    // Sleep roosts against the physical desktop edge, never an application
    // window. The thin rectangle exposes the bottom edge as horizontal support.
    surfaces.push(SurfaceCandidate {
        surface_id: SurfaceId("screen:bottom_edge".into()),
        // One physical pixel above the desktop bottom keeps the rectangle
        // non-degenerate while making the locked silhouette target land inside
        // the independent +/-4 px acceptance band on every resolution.
        minimum: Vec2::new(0.0, 1.0 - 1.0 / desktop_height),
        maximum: Vec2::ONE,
        velocity: Vec2::ZERO,
        familiarity: 1.0,
        recent_failed_landings: 0,
    ...
    BehaviorContextFrame {
        ...
        timestamp_seconds: runtime.sensors.timestamp,
        body,
        somatic,
        cursor_position: runtime.sensors.cursor_position,
        cursor_velocity: runtime.sensors.cursor_velocity,
        cursor_acceleration: runtime.sensors.cursor_acceleration,
        pointer_down: runtime.sensors.pointer_down,
        pointer_pressed: runtime.sensors.pointer_pressed,
        pointer_released: runtime.sensors.pointer_released,
        pet_touched: runtime.sensors.pet_touched,
        pet_dragged: runtime.sensors.pet_dragged,
        selected_salience: runtime
            .vita
            .visual_attention_target()
            .map_or(0.0, |target| target.score),
        companion_intent: runtime.vita.companion_intent().primary,
        companion_confidence: runtime.vita.companion_intent().confidence,
        gesture: gesture.kind,
        gesture_confidence: gesture.confidence,
        gesture_ended: gesture.ended,
        boundary_violation: snapshot
            .felt
            .pain_like
            .max(snapshot.felt.restraint)
            .max((body.contact.pressure - 0.55).max(0.0) / 0.45)
            .clamp(0.0, 1.0),
        locomotion_completed: runtime.body.simulation.feedback.locomotion_completed,
        body_bottom_extent,
        screen_edge_gap_px: runtime.screen_edge_gap_px,
        screen_edge_normal_velocity_px_s: runtime.screen_velocity_px.y,
        screen_edge_support_stable_seconds: runtime.screen_edge_support_stable_seconds,
        screen_edge_supported: runtime.screen_edge_supported,
        surfaces,
