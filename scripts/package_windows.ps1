...
cargo build --manifest-path (Join-Path $workspace 'Cargo.toml') --release --target $Target -p pet2 -p body_lab -p habitat_lab -p voice_lab
...
Copy-Item -Force (Join-Path $workspace "target/$Target/release/voice_lab.exe") (Join-Path $dist 'tools/VoiceLab.exe')
...
$packagedLab = Join-Path $dist 'Pet2 Dev Console.exe'
