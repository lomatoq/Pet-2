...
impl BehaviorPerformanceRuntime {
    ...
    pub fn cancel_social_request(&mut self) {
        if self
            .active
            .as_ref()
            .is_some_and(|a| a.program.requests_user_attention())
        {
            self.finish_active(CompletionReason::GracefulWithdrawal);
        }
        self.acknowledgement_seconds = 0.0;
        self.ambiguity_seconds = 0.0;
    }
...
mod tests {
    ...
    #[test]
    fn repeated_pain_interrupts_travel_even_during_defense_cooldown() {
        let mut goal = goal(ActionId::ExploreScreen, Vec2::new(0.9, 0.5));
        let context = BehaviorContextFrame::default();
        let mut runtime = BehaviorPerformanceRuntime::new(9);
        runtime.begin_lab_fixture(BehaviorProgramId::MovePunctuatedTravel, &goal, &context);
        runtime.cooldowns[BehaviorProgramId::DefenseLocalPainGuard.index()] = 1.0;
        goal.felt.pain_like = 0.9;
    ...
    fn focus_or_boundary_cancels_social_bids_before_the_wait_phase() {
        ...
        for program in BehaviorProgramId::ALL
            .into_iter()
            .filter(|p| p.requests_user_attention())
        {
            for focus in [true, false] {
                ...
                context.focus_mode = focus;
                context.boundary_violation = if focus { 0.0 } else { 0.2 };
                let packet = runtime.tick_lab_fixture(&goal, &context, 0.01);
                assert!(
                    packet.program.is_none(),
