...
/// Single simulation writer, bounded lock-free audio reader. All payload and
/// sequence operations share one total order, including on Apple Silicon, so
/// an accepted snapshot cannot mix fields from different physics frames.
...
impl BodyVoiceBridge {
    pub fn publish(&self, frame: BodyVoiceFrame) {
        ...
        self.component_count
            .store(u32::from(frame.component_count), Ordering::SeqCst);
    ...
    pub fn snapshot_or(&self, fallback: BodyVoiceFrame) -> BodyVoiceFrame {
        for _ in 0..4 {
            ...
            let component_count = self.component_count.load(Ordering::SeqCst) as u8;
            let values = std::array::from_fn(|index| {
                f32::from_bits(self.fields[index].load(Ordering::SeqCst))
            });
...
mod tests {
    ...
    #[test]
    fn concurrent_audio_snapshots_never_mix_physics_frames() {
        let bridge = Arc::new(BodyVoiceBridge::default());
        let writer = Arc::clone(&bridge);
        let worker = std::thread::spawn(move || {
            for index in 0..20_000 {
                let value = (index % 100) as f32 / 100.0;
                writer.publish(BodyVoiceFrame {
                    stretch: value,
                    compression: value,
                    bond_strain: value,
                    ..BodyVoiceFrame::default()
        ...
        let mut previous = BodyVoiceFrame::default();
        for _ in 0..20_000 {
            let frame = bridge.snapshot_or(previous);
            assert_eq!(frame.stretch, frame.compression);
            assert_eq!(frame.stretch, frame.bond_strain);
            previous = frame;
        }
        worker.join().unwrap();
    }
