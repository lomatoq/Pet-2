...
mod tests {
    ...
    fn efference_includes_near_target_braking() {
        ...
        body.feedback.world_position = Vec2::splat(0.5);
        body.feedback.velocity = Vec2::new(0.5, 0.0);
        let intent = BodyIntent {
            locomotion: LocomotionMode::Arrive,
            target_position: Vec2::new(0.5001, 0.5),
            desired_speed: 0.5,
            target_surface: None,
        ...
        let command = body.preview_motor_velocity(
            &genome.body,
            &intent,
            &SensorFrame::default(),
            1.0 / 120.0,
        );
        // Remaining travel is inside the acceleration-slew braking reserve.
        assert_eq!(command, Vec2::ZERO);
        body.fixed_update(&genome.body, &intent, &SensorFrame::default(), 1.0 / 120.0);
        assert_eq!(body.motor_velocity, command);
    }
